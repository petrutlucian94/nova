...
class IOUtils(object):
    ...
    def _run_and_check_output(self, func, *args, **kwargs):
        ...
        error_codes = kwargs.pop('error_codes', [None])
    ...
    def handle_last_error(self, ignored_error_codes=[]):
        ...
        err = (_LE("IO operation failed. Error code: %(error_code)s. "
    ...
    def read(self, *args, **kwargs):
        func = kernel32.ReadFileEx
        self._io_operation(func, *args, **kwargs)

    def write(self, *args, **kwargs):
        func = kernel32.WriteFileEx
        self._io_operation(func, *args, **kwargs)

    def _io_operation(self, func, handle, buff, num_bytes,
                      overlapped_structure,
                      completion_routine):
        self._reset_event(overlapped_structure.hEvent)

        self._run_and_check_output(func, handle, buff, num_bytes,
...
class IOQueue(Queue.Queue):
    ...
    def get(self, timeout=IO_QUEUE_TIMEOUT, continue_on_timeout=True):
        while self._client_connected.isSet():
            try:
                data = Queue.Queue.get(self, timeout=timeout)
                return data
            except Queue.Empty:
    ...
    def get_burst(self, timeout=IO_QUEUE_TIMEOUT,
        ...
        while self._client_connected and data and not (len(data) > max_size):
