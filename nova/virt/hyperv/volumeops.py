...
class VolumeOps(object):
    ...
    def attach_volume(self, connection_info, instance_name, ebs_root=False):
        ...
        qos_specs = connection_info['data'].get('qos_specs', {})
    ...
    def parse_disk_qos_specs(self, qos_specs):
        total_bytes_sec = qos_specs.get('total_bytes_sec')
        min_bytes_sec = qos_specs.get('min_iops_sec')

        total_iops = qos_specs.get('total_iops_sec',
                                   self._bytes_per_sec_to_iops(
                                       total_bytes_sec))
        min_iops = qos_specs.get('min_iops_sec',
                                 self._bytes_per_sec_to_iops(min_bytes_sec))

        if total_iops and min_iops and total_iops < min_iops:
    ...
    def _bytes_per_sec_to_iops(self, no_bytes):
        ...
        iops = ((no_bytes + self._IOPS_BASE_SIZE - 1) /
                self._IOPS_BASE_SIZE if no_bytes else 0)
        return iops
